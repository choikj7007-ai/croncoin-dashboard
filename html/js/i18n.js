...
const I18N = {
    en: {
        ...
        'guide.wallet.desc': 'Generate a complete HD (Hierarchical Deterministic) wallet following the standard flow. Click "Generate New Wallet" to create a wallet from scratch — all 7 steps are displayed progressively. Step 1 (Entropy): 128-bit random number, the source of all security. Step 2 (Mnemonic/BIP-39): 12 words derived from entropy + checksum, the human-readable backup. Step 3 (Master Key/BIP-32): Mnemonic + HMAC-SHA512 produces two keys simultaneously — master xprv (extended private key) and master xpub (extended public key). Step 4 (HD Derivation/BIP-44): Each derivation level (m/84h/1h/0h/0/0) generates an xprv + xpub pair. Deriving from xprv produces child xprv + xpub; deriving from xpub produces child xpub only (no private key — useful for watch-only wallets). Step 5 (Private Key): WIF private key extracted from the final child xprv. Step 6 (Public Key): Compressed public key from secp256k1 elliptic curve. Step 7 (Address): Public Key → SHA-256 → RIPEMD-160 → Bech32 encoding. Important: Back up your 12-word mnemonic — it is the only way to restore the wallet.',
    ...
    ko: {
        ...
        'hd.guideNotice': '<strong>HD 지갑 생성</strong><ul><li>완전한 HD(계층적 결정성) 지갑을 처음부터 생성합니다.</li><li>과정은 표준을 따릅니다: <em>엔트로피 → 니모닉 → 마스터키 → HD 파생 → 개인키 → 공개키 → 주소</em></li><li><em>니모닉(12단어)</em>은 지갑의 사람이 읽을 수 있는 백업입니다. 안전하게 보관하세요.</li><li>하나의 니모닉에서 무제한의 주소를 파생할 수 있습니다.</li></ul>',
        'hd.step1': '엔트로피',
        ...
        'guide.wallet.desc': '표준 HD(계층적 결정성) 지갑 생성 과정을 따라 완전한 지갑을 생성합니다. "새 지갑 생성하기" 버튼을 클릭하면 처음부터 지갑이 생성되며 7단계가 순차적으로 표시됩니다. 1단계(엔트로피): 128비트 난수, 모든 보안의 원천. 2단계(니모닉/BIP-39): 엔트로피 + 체크섬에서 파생된 12단어, 사람이 읽을 수 있는 백업. 3단계(마스터키/BIP-32): 니모닉 + HMAC-SHA512로 두 개의 키가 동시에 생성 — 마스터 xprv(확장 개인키)와 마스터 xpub(확장 공개키). 4단계(HD 파생/BIP-44): 각 파생 단계(m/84h/1h/0h/0/0)마다 xprv + xpub 쌍이 생성됩니다. xprv로 파생하면 자식 xprv + xpub 둘 다 생성되고, xpub로 파생하면 자식 xpub만 생성됩니다(개인키 없음 — 조회 전용 지갑에 유용). 5단계(개인키): 최종 자식 xprv에서 추출한 WIF 개인키. 6단계(공개키): secp256k1 타원곡선으로 생성된 압축 공개키. 7단계(주소): 공개키 → SHA-256 → RIPEMD-160 → Bech32 인코딩. 중요: 12단어 니모닉을 반드시 백업하세요 — 지갑을 복원할 수 있는 유일한 방법입니다.',
